package com.hexagram2021.misc_twf_zombie_animals.common.entity;
...
public class ZombieRabbitEntity extends ZombieAnimalEntity<Rabbit> {
	...
	public float getJumpCompletion(float tick) {
		return this.jumpDuration == 0 ? 0.0F : (this.jumpTicks + tick) / this.jumpDuration;
	}
	...
	class RaidGardenGoal extends MoveToBlockGoal {
		...
		public boolean canUse() {
			if (this.nextStartTick <= 0) {
				if (!net.neoforged.neoforge.event.EventHooks.canEntityGrief(ZombieRabbitEntity.this.level(), ZombieRabbitEntity.this)) {
					return false;
				}
		...
		public void tick() {
			super.tick();
			ZombieRabbitEntity.this.getLookControl().setLookAt(this.blockPos.getX() + 0.5D, this.blockPos.getY() + 1.0D, this.blockPos.getZ() + 0.5D, 10.0F, ZombieRabbitEntity.this.getMaxHeadXRot());
		...
		protected boolean isValidTarget(LevelReader level, BlockPos blockPos) {
			...
			if (blockstate.is(Blocks.FARMLAND) && this.wantsToRaid && !this.canRaid) {
				...
				if (blockstate.getBlock() instanceof CarrotBlock carrotBlock && carrotBlock.isMaxAge(blockstate)) {
					this.canRaid = true;
					return true;
				}
			}
