...
public class ZombieAnimalEntity<T extends Animal> extends Zombie {
	...
	public void tick() {
		if (!this.level().isClientSide && this.isAlive() && this.isConverting()) {
			...
			if (this.conversionTime <= 0 && net.neoforged.neoforge.event.EventHooks.canLivingConvert(this, EntityType.VILLAGER, timer -> this.conversionTime = timer)) {
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if(!MISCTWFZombieAnimalsConfig.ZOMBIE_ANIMALS_CAN_BE_HEALED.get() || !itemstack.is(Items.GOLDEN_APPLE) || this.animalEntityType == null) {
	...
	private void finishConversion(ServerLevel level) {
		...
		animal.finalizeSpawn(level, level.getCurrentDifficultyAt(animal.blockPosition()), MobSpawnType.CONVERSION, null);
		this.extraConversion(animal);
		if (!this.isSilent()) {
			level.levelEvent(null, LevelEvent.SOUND_ZOMBIE_CONVERTED, this.blockPosition(), 0);
		}
		net.neoforged.neoforge.event.EventHooks.onLivingConvert(this, animal);
	}
