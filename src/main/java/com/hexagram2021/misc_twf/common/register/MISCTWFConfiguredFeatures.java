...
/**
 * 模组已配置特征注册类，负责注册怪物蛋等世界生成特征的配置喵~
 * 定义怪物蛋特征的生成权重和可生成的实体类型，支持与 zombie_extreme 模组联动喵~
 *
 * @author liudongyu
 */
public final class MISCTWFConfiguredFeatures {
	public static final Holder<ConfiguredFeature<?, ?>> MONSTER_EGG = BuiltinRegistries.register(
			BuiltinRegistries.CONFIGURED_FEATURE, "monster_egg", new ConfiguredFeature<>(MISCTWFFeatures.MONSTER_EGG, new MonsterEggFeature.MonsterEggFeatureConfiguration(
					Util.make(() -> {
						ImmutableList.Builder<MonsterEggEntries.MonsterEggEntry> builder = ImmutableList.builder();
						builder.add(new MonsterEggEntries.MonsterEggEntry(EntityType.ZOMBIE, Weight.of(6)));
						...
						if(NIGHT_HUNTER != null) {
							builder.add(new MonsterEggEntries.MonsterEggEntry(NIGHT_HUNTER, Weight.of(5)));
						}
						...
						if(INFECTED_JUGGERNAUT != null) {
							builder.add(new MonsterEggEntries.MonsterEggEntry(INFECTED_JUGGERNAUT, Weight.of(2)));
						}
						...
						if(INFECTED_HAZMAT != null) {
							builder.add(new MonsterEggEntries.MonsterEggEntry(INFECTED_HAZMAT, Weight.of(2)));
						}
						...
						if(PATIENT_ZERO != null) {
							builder.add(new MonsterEggEntries.MonsterEggEntry(PATIENT_ZERO, Weight.of(2)));
						}
